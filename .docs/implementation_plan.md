...
### File Upload
#### [NEW] [schema.prisma](file:///d:/dev/catalog/prisma/schema.prisma)
- **AssetAttachment**: `id`, `assetId`, `url`, `type`, `name`, `size`.
...
### Usage Request
#### [NEW] [route.ts](file:///d:/dev/catalog/src/app/api/assets/[id]/request/route.ts)
- `POST` to create `UsageHistory`.
- Increments `asset.usageCount`.

### Reviews & Ratings
#### [NEW] [route.ts](file:///d:/dev/catalog/src/app/api/assets/[id]/reviews/route.ts)
- `POST` to create `Review`.
- `GET` to fetch reviews (optional, can use server component).

#### [NEW] [component](file:///d:/dev/catalog/src/components/assets/review-section.tsx)
...
## UI/UX Polish (ServiceNow Style)
#### [MODIFY] [globals.css](file:///d:/dev/catalog/src/app/globals.css)
- **Theme**: Professional "Enterprise" palette (Navy Blue, Slate, White).
- **Typography**: Inter or similar clean sans-serif.

#### [MODIFY] [navbar.tsx](file:///d:/dev/catalog/src/components/layout/navbar.tsx)
- **Style**: Sticky, glassmorphism, clear navigation.

#### [MODIFY] [page.tsx](file:///d:/dev/catalog/src/app/page.tsx)
- **Hero Section**: Welcoming banner with search.
- **Card Design**: Clean borders, hover shadows, status indicators.

#### [MODIFY] [page.tsx](file:///d:/dev/catalog/src/app/assets/[id]/page.tsx)
- **Layout**: Sidebar for metadata, main content for description.
- **Tabs**: Overview, Reviews, Version History.
